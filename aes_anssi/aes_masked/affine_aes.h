...
typedef struct _PACKED 
{
	...
	UCHAR                             key_masksState[16*11];	  // The corresponding mask state
	...
	UCHAR                             key_gtab[16*16];		  // LUT for the masking affine function G
	UCHAR                             key_maskedSBox[16*16];	  // LUT for the masked AES SBox
	UCHAR                             key_permIndicesMC[4];           // LUT for the permIndices function
	UCHAR  				  key_rcon_mult[40];     	  // LUT for rcon multiplied by rmult
}  STRUCT_KEY_CONTEXT;
#endif
